...
6. **Verify all three layers.** Every test must verify: (a) context menu state (enabled/disabled items), (b) controller pipeline state (processed events, resources), and (c) view-layer column structure (derived columns, micro-columns, variants).
