...
for (const key of weaponContext.keys()) {
  // Match only base weapon files (e.g. ./forgeborn-scathe/forgeborn-scathe.json)
  // Skip skills/, statuses/ subdirs and generic/
  if (key.includes('/skills/') || key.includes('/statuses/') || key.includes('/generic/')) continue;
  // Skip effects files
  if (key.includes('-effects.json')) continue;
