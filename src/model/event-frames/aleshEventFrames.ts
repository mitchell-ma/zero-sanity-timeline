...
const OP = skillsData.operators.ALESH;

class AleshSkillEventFrame extends SkillEventFrame {
...
class AleshSkillEventSequence extends SkillEventSequence {
  private readonly _durationSeconds: number;
  private readonly _frames: readonly AleshSkillEventFrame[];
  ...
  constructor(seqData: Record<string, any>, prefix: string) {
    ...
    const frames: AleshSkillEventFrame[] = [];
    for (let i = 1; i <= tickCount; i++) {
      const tick = seqData[`${prefix}_TICK_${i}`];
      if (tick) frames.push(new AleshSkillEventFrame(tick));
    }
    this._frames = frames;
  }

  getDurationSeconds(): number { return this._durationSeconds; }
  getFrames(): readonly AleshSkillEventFrame[] { return this._frames; }
}

// ── Basic Attack ────────────────────────────────────────────────────────────
export const ALESH_BASIC_ATTACK_SEQUENCES: readonly SkillEventSequence[] = [1, 2, 3, 4, 5]
  .map((n) => {
    const key = `ALESH_BASIC_ATTACK_SEQUENCE_${n}`;
    return new AleshSkillEventSequence(
...
// ── Battle Skill ────────────────────────────────────────────────────────────
export const ALESH_BATTLE_SKILL_SEQUENCE: SkillEventSequence = new AleshSkillEventSequence(
  OP.BATTLE_SKILL.ALESH_BATTLE_SKILL,
  'ALESH_BATTLE_SKILL',
);

// ── Combo Skill ─────────────────────────────────────────────────────────────
export const ALESH_COMBO_SKILL_SEQUENCE: SkillEventSequence = new AleshSkillEventSequence(
