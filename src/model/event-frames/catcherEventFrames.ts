...
const OP = skillsData.operators.CATCHER;

class CatcherSkillEventFrame extends SkillEventFrame {
...
class CatcherSkillEventSequence extends SkillEventSequence {
  private readonly _durationSeconds: number;
  private readonly _frames: readonly CatcherSkillEventFrame[];
  ...
  constructor(seqData: Record<string, any>, prefix: string) {
    ...
    const frames: CatcherSkillEventFrame[] = [];
    for (let i = 1; i <= tickCount; i++) {
      const tick = seqData[`${prefix}_TICK_${i}`];
      if (tick) frames.push(new CatcherSkillEventFrame(tick));
    }
    this._frames = frames;
  }

  getDurationSeconds(): number { return this._durationSeconds; }
  getFrames(): readonly CatcherSkillEventFrame[] { return this._frames; }
}

// ── Basic Attack ────────────────────────────────────────────────────────────
export const CATCHER_BASIC_ATTACK_SEQUENCES: readonly SkillEventSequence[] = [1, 2, 3, 4]
  .map((n) => {
    const key = `CATCHER_BASIC_ATTACK_SEQUENCE_${n}`;
    return new CatcherSkillEventSequence(
...
// ── Battle Skill ────────────────────────────────────────────────────────────
export const CATCHER_BATTLE_SKILL_SEQUENCE: SkillEventSequence = new CatcherSkillEventSequence(
  OP.BATTLE_SKILL.CATCHER_BATTLE_SKILL,
  'CATCHER_BATTLE_SKILL',
);

// ── Combo Skill ─────────────────────────────────────────────────────────────
export const CATCHER_COMBO_SKILL_SEQUENCE: SkillEventSequence = new CatcherSkillEventSequence(
