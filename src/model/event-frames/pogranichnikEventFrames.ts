...
const OP = skillsData.operators.POGRANICHNK;

class PogranichnikSkillEventFrame extends SkillEventFrame {
...
class PogranichnikSkillEventSequence extends SkillEventSequence {
  private readonly _durationSeconds: number;
  private readonly _frames: readonly PogranichnikSkillEventFrame[];
  ...
  constructor(seqData: Record<string, any>, prefix: string) {
    ...
    const frames: PogranichnikSkillEventFrame[] = [];
    for (let i = 1; i <= tickCount; i++) {
      const tick = seqData[`${prefix}_TICK_${i}`];
      if (tick) frames.push(new PogranichnikSkillEventFrame(tick));
    }
    this._frames = frames;
  }

  getDurationSeconds(): number { return this._durationSeconds; }
  getFrames(): readonly PogranichnikSkillEventFrame[] { return this._frames; }
}

// ── Basic Attack ────────────────────────────────────────────────────────────
export const POGRANICHNK_BASIC_ATTACK_SEQUENCES: readonly SkillEventSequence[] = [1, 2, 3, 4, 5]
  .map((n) => {
    const key = `POGRANICHNK_BASIC_ATTACK_SEQUENCE_${n}`;
    return new PogranichnikSkillEventSequence(
...
// ── Battle Skill ────────────────────────────────────────────────────────────
export const POGRANICHNK_BATTLE_SKILL_SEQUENCE: SkillEventSequence = new PogranichnikSkillEventSequence(
  OP.BATTLE_SKILL.POGRANICHNK_BATTLE_SKILL,
  'POGRANICHNK_BATTLE_SKILL',
);

// ── Combo Skill ─────────────────────────────────────────────────────────────
export const POGRANICHNK_COMBO_SKILL_SEQUENCE: SkillEventSequence = new PogranichnikSkillEventSequence(
