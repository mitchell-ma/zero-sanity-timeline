  ...
  it('user adds Akekuri at 5s then Antal at 2.5s — all 3 clamped correctly', () => {
    ...
    act(() => {
      result.current.handleAddEvent(
        ...
        { id: CombatSkillType.CONTROL, name: CombatSkillType.CONTROL, segments: [{ properties: { duration: TOTAL_FRAMES - swapAkekuri, name: 'Control' } }] },
      );
    });
    act(() => {
      result.current.handleAddEvent(
        SLOT_2, OPERATOR_COLUMNS.INPUT, swapAntal,
        { id: CombatSkillType.CONTROL, name: CombatSkillType.CONTROL, segments: [{ properties: { duration: TOTAL_FRAMES - swapAntal, name: 'Control' } }] },
      );
    });
