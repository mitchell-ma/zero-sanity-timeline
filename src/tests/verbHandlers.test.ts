...
function derivedEvents(result: TimelineEvent[], inputEvents: TimelineEvent[]) {
  const inputIds = new Set(inputEvents.map(e => e.id));
  return result.filter(ev => !inputIds.has(ev.id));
}
...
describe('Khravengger — APPLY CRYO INFLICTION trigger', () => {
  ...
  test('cryo infliction on enemy triggers Bonechilling buff', () => {
    ...
    const buffs = derivedEvents(result, events).filter(ev => ev.name === 'KHRAVENGGER_BONECHILLING');
    expect(buffs.length).toBe(1);
...
describe('Swordmancer — APPLY STATUS trigger', () => {
  ...
  test('applying a status triggers Swordmancer effect on enemy', () => {
    ...
    const effects = derivedEvents(result, events).filter(ev => ev.name === 'SWORDMANCER');
...
describe('Former Finery — ENEMY HIT OPERATOR trigger', () => {
  ...
  test('first proc at frame 0, self-buff on operator', () => {
    ...
    const buffs = result.filter(ev => ev.name === 'FORMER_FINERY_MINCING_THERAPY');
  ...
  function makeTriggerDef(toObject?: string, toDeterminer?: string) {
    ...
    return [{
      properties: {
        ...
        statusLevel: { limit: { verb: 'IS', value: 1 }, statusLevelInteractionType: 'RESET' },
        duration: { value: [10], unit: 'SECOND' },
      },
