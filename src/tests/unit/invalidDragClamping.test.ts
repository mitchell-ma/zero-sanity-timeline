...
describe('clampDeltaByResourceZones — normal clamping', () => {
  const events = [makeEvent({ uid: 'e1', columnId: SKILL_COLUMNS.BATTLE, startFrame: 50 })];
  // Zone at [100, 200)
  ...
  test('blocks dragging up into a zone', () => {
    ...
    const ev = [makeEvent({ uid: 'e1', columnId: SKILL_COLUMNS.BATTLE, startFrame: 250 })];
...
describe('clampDeltaByResourceZones — invalid at drag start', () => {
  ...
  const events = [makeEvent({ uid: 'e1', columnId: SKILL_COLUMNS.BATTLE, startFrame })];
  const zones = makeZones([100, 200]);
...
describe('clampDeltaByComboWindow — invalid at drag start', () => {
  ...
  // Window at [100, 200).
  const startFrame = 50;
  const comboEvent = makeEvent({ uid: 'c1', columnId: SKILL_COLUMNS.COMBO, startFrame });
...
describe('clampDeltaByComboWindow — normal clamping', () => {
  ...
  test('non-combo events pass through', () => {
    const battleEvent = makeEvent({ uid: 'b1', columnId: SKILL_COLUMNS.BATTLE, startFrame: 150 });
...
describe('clampDeltaByOverlap — normal clamping', () => {
  ...
  const events = [
    makeEvent({ uid: 'e1', columnId: SKILL_COLUMNS.BATTLE, startFrame: 50, nonOverlappableRange: 60 }),
    makeEvent({ uid: 'e2', columnId: SKILL_COLUMNS.BATTLE, startFrame: 200, nonOverlappableRange: 60 }),
  ];
  ...
  test('events in different columns are not blocked', () => {
    const mixedEvents = [
      makeEvent({ uid: 'e1', columnId: SKILL_COLUMNS.BATTLE, startFrame: 50, nonOverlappableRange: 60 }),
      makeEvent({ uid: 'e2', columnId: SKILL_COLUMNS.ULTIMATE, startFrame: 60, nonOverlappableRange: 60 }),
    ];
...
describe('clampDeltaByOverlap — invalid at drag start', () => {
  ...
  const events = [
    makeEvent({ uid: 'e1', columnId: SKILL_COLUMNS.BATTLE, startFrame: 100, nonOverlappableRange: 60 }),
    makeEvent({ uid: 'e2', columnId: SKILL_COLUMNS.BATTLE, startFrame: 120, nonOverlappableRange: 60 }),
  ];
  const dragSet = new Set(['e1']);
...
describe('clampDeltaByOverlap — full lifecycle', () => {
  ...
  const events = [
    makeEvent({ uid: 'e1', columnId: SKILL_COLUMNS.BATTLE, startFrame: 150, nonOverlappableRange: 60 }),
    makeEvent({ uid: 'e2', columnId: SKILL_COLUMNS.BATTLE, startFrame: 140, nonOverlappableRange: 60 }),
  ];
  const dragSet = new Set(['e1']);
  const overlapInvalid = new Set(['e1']);
  const overlapReval = new Set<string>();
...
describe('clampDeltaByOverlap — edge cases', () => {
  test('zero-range events pass through', () => {
    const events = [
      makeEvent({ uid: 'e1', columnId: SKILL_COLUMNS.BATTLE, startFrame: 50, nonOverlappableRange: 0 }),
      makeEvent({ uid: 'e2', columnId: SKILL_COLUMNS.BATTLE, startFrame: 60, nonOverlappableRange: 60 }),
    ];
  ...
  test('different owners do not block each other', () => {
    const events = [
      makeEvent({ uid: 'e1', columnId: SKILL_COLUMNS.BATTLE, startFrame: 50, ownerId: 'op-1', nonOverlappableRange: 60 }),
      makeEvent({ uid: 'e2', columnId: SKILL_COLUMNS.BATTLE, startFrame: 80, ownerId: 'op-2', nonOverlappableRange: 60 }),
    ];
...
describe('clampDeltaByOverlap — multiple siblings', () => {
  ...
  const events = [
    makeEvent({ uid: 'e1', columnId: SKILL_COLUMNS.BATTLE, startFrame: 50, nonOverlappableRange: 60 }),
    makeEvent({ uid: 'e2', columnId: SKILL_COLUMNS.BATTLE, startFrame: 200, nonOverlappableRange: 60 }),
    makeEvent({ uid: 'e3', columnId: SKILL_COLUMNS.BATTLE, startFrame: 400, nonOverlappableRange: 60 }),
  ];
  const dragSet = new Set(['e1']);
  ...
  test('invalid at drag start: free movement through multiple overlapping siblings', () => {
    ...
    const overlappingEvents = [
      makeEvent({ uid: 'e1', columnId: SKILL_COLUMNS.BATTLE, startFrame: 210, nonOverlappableRange: 60 }),
      makeEvent({ uid: 'e2', columnId: SKILL_COLUMNS.BATTLE, startFrame: 200, nonOverlappableRange: 60 }),
      makeEvent({ uid: 'e3', columnId: SKILL_COLUMNS.BATTLE, startFrame: 400, nonOverlappableRange: 60 }),
    ];
    const overlapInvalid = new Set(['e1']);
...
describe('clampDeltaByOverlap — revalidated bidirectional clamping', () => {
  ...
  const events = [
    makeEvent({ uid: 'e1', columnId: SKILL_COLUMNS.BATTLE, startFrame: 200, nonOverlappableRange: 60 }),
    makeEvent({ uid: 'e2', columnId: SKILL_COLUMNS.BATTLE, startFrame: 100, nonOverlappableRange: 60 }),
    makeEvent({ uid: 'e3', columnId: SKILL_COLUMNS.BATTLE, startFrame: 300, nonOverlappableRange: 60 }),
  ];
  const dragSet = new Set(['e1']);
  const overlapInvalid = new Set<string>();
  const overlapReval = new Set(['e1']);
