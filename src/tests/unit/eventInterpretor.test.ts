...
describe('EventInterpretorController: ALL', () => {
  test('ALL FOR LESS_THAN_EQUAL 4: CONSUME + APPLY (absorption pattern)', () => {
    ...
    const consumed = interp.controller.getAllEvents().filter(ev =>
...
describe('EventInterpretorController: ANY', () => {
  ...
  test('ANY returns false when no predicates match', () => {
    ...
    const result = interp.interpret(effect, ctx);
    expect(result).toBe(false);
    expect(interp.controller.getAllEvents().length).toBe(0);
  });
});
...
describe('EventInterpretorController: interpretEffects', () => {
  test('executes multiple effects sequentially', () => {
    ...
    expect(result).toBe(true);
    expect(interp.controller.getAllEvents().length).toBe(2);
    expect(interp.controller.getAllEvents()[0].columnId).toBe(INFLICTION_COLUMNS.HEAT);
    expect(interp.controller.getAllEvents()[1].columnId).toBe(INFLICTION_COLUMNS.HEAT);
  });
});
...
describe('EventInterpretorController: APPLY LIFT STATUS (PHYSICAL)', () => {
  ...
  test('no Vulnerable → adds Vulnerable only, no Lift status', () => {
    ...
    const vulnEvents = interp.controller.getAllEvents().filter(
      ev => ev.columnId === PHYSICAL_INFLICTION_COLUMNS.VULNERABLE,
    );
    const liftEvents = interp.controller.getAllEvents().filter(
  ...
  test('Lift status has 1 segment with damage frame at offset 0', () => {
    ...
    expect(findDealDamageInClauses(segments![0].frames![0].clause)!.values[0]).toBe(1.2);
  });
...
describe('EventInterpretorController: APPLY KNOCK_DOWN STATUS (PHYSICAL)', () => {
  ...
  test('no Vulnerable → adds Vulnerable only, no Knock Down status', () => {
    ...
    const kdEvents = interp.controller.getAllEvents().filter(
  ...
  test('Vulnerable active → adds Vulnerable + creates Knock Down status', () => {
    ...
    expect(findDealDamageInClauses(kdEvents[0].segments![0].frames![0].clause)!.values[0]).toBe(1.2);
  });
...
describe('EventInterpretorController: APPLY CRUSH STATUS (PHYSICAL)', () => {
  ...
  test('no Vulnerable → adds Vulnerable only, no Crush status', () => {
    ...
    const crushEvents = interp.controller.getAllEvents().filter(
  ...
  test('1 Vulnerable → consumes it, creates Crush with 300% multiplier', () => {
    ...
    expect(findDealDamageInClauses(crushEvents[0].segments![0].frames![0].clause)!.values[0]).toBe(3.0);
  ...
  test('2 Vulnerable → consumes all, creates Crush with 450% multiplier', () => {
    ...
    expect(crushEvents.length).toBe(1);
    expect(findDealDamageInClauses(crushEvents[0].segments![0].frames![0].clause)!.values[0]).toBe(4.5);
  ...
  test('3 Vulnerable → consumes all, creates Crush with 600% multiplier', () => {
    ...
    expect(crushEvents.length).toBe(1);
    expect(findDealDamageInClauses(crushEvents[0].segments![0].frames![0].clause)!.values[0]).toBe(6.0);
  ...
  test('4 Vulnerable → consumes all, creates Crush with 750% multiplier', () => {
    ...
    expect(crushEvents.length).toBe(1);
    expect(findDealDamageInClauses(crushEvents[0].segments![0].frames![0].clause)!.values[0]).toBe(7.5);
  ...
  test('Crush has no stagger value', () => {
    ...
    expect(findStaggerInClauses(crushEvents[0].segments![0].frames![0].clause)).toBeUndefined();
  });
});
...
describe('EventInterpretorController: APPLY BREACH STATUS (PHYSICAL)', () => {
  ...
  test('no Vulnerable → adds Vulnerable only, no Breach status', () => {
    ...
    const breachEvents = interp.controller.getAllEvents().filter(
  ...
  test('1 Vulnerable → consumes, Breach with 100% multiplier and 12s duration', () => {
    ...
    expect(findDealDamageInClauses(breachEvents[0].segments![0].frames![0].clause)!.values[0]).toBe(1.0);
  });
  ...
  test('2 Vulnerable → 150% multiplier and 18s duration', () => {
    ...
    expect(findDealDamageInClauses(breachEvents[0].segments![0].frames![0].clause)!.values[0]).toBe(1.5);
  });
  ...
  test('3 Vulnerable → 200% multiplier and 24s duration', () => {
    ...
    expect(findDealDamageInClauses(breachEvents[0].segments![0].frames![0].clause)!.values[0]).toBe(2.0);
  });
  ...
  test('4 Vulnerable → 250% multiplier and 30s duration', () => {
    ...
    expect(findDealDamageInClauses(breachEvents[0].segments![0].frames![0].clause)!.values[0]).toBe(2.5);
  });
  ...
  test('Breach has no stagger', () => {
    ...
    expect(findStaggerInClauses(breachEvents[0].segments![0].frames![0].clause)).toBeUndefined();
  });
});
...
describe('EventInterpretorController: sourceEventUid uid propagation', () => {
  test('sourceEventUid set with matching column: child inherits parent uid', () => {
    ...
    interp.interpret(effect, ctx);
    expect(interp.controller.getAllEvents()).toHaveLength(1);
    expect(interp.controller.getAllEvents()[0].uid).toBe('parent-uid');
  });
  ...
  test('sourceEventUid set with mismatched column: child gets fresh derived uid', () => {
    ...
    interp.interpret(effect, ctx);
    expect(interp.controller.getAllEvents()).toHaveLength(1);
    expect(interp.controller.getAllEvents()[0].uid).not.toBe('parent-uid');
    expect(interp.controller.getAllEvents()[0].uid).toContain('NATURE_INFLICTION');
  });
  ...
  test('no sourceEventUid: child gets unique derived uid', () => {
    ...
    interp.interpret(effect, ctx);
    expect(interp.controller.getAllEvents()).toHaveLength(1);
    expect(interp.controller.getAllEvents()[0].uid).toContain('CRYO_INFLICTION');
  });
  ...
  test('sourceEventUid set with matching column: status inherits parent uid', () => {
    ...
    interp.interpret(effect, ctx);
    expect(interp.controller.getAllEvents()).toHaveLength(1);
    expect(interp.controller.getAllEvents()[0].uid).toBe('parent-status-uid');
  });
});
