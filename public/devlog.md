...
## 2026-04-18
- **Real-time collaboration — share a session live with friends.** A new **Collab** button in the top bar lets you **Host** a room (you'll get a room code to share) or **Join** one. Once connected, every loadout edit — adds, drags, edits, view permutations — streams between peers automatically over a peer-to-peer connection (no server in the middle). The button shows a coloured dot for connection status (green = connected, amber = connecting, orange = reconnecting with a "next try in Ns" countdown, red = error). The dropdown lists every peer in the room and a leave-room action. If your connection drops, the app keeps retrying with backoff and will resync on reconnect. Loadouts shared with collaborators get a small people icon in the sidebar so you can tell at a glance which ones are syncing.
- **New Statistics view — compare any number of loadouts side by side.** A new **Statistics** tab in the left rail lets you create statistics sheets that pull data from multiple loadouts (or permutation views) at once. Each sheet shows per-source rows broken out by Basic / Battle / Combo / Ultimate totals, Team DPS, Duration, Time to Kill, Crowd Control %, and Team Total — plus per-operator subrows. Filter columns and operators independently per sheet. Switch the **Comparison Mode** between **Raw** values, **Δ vs base** (each row's % change vs the first row), or **Δ vs previous** (each row's % change vs the row above). When every source in a sheet shares the same parent loadout, the view automatically groups them as a single permutation table — perfect for "how does this rotation scale across P0 – P5 / R1 – R9?". Crit Mode (Expected / Never / Always) is a per-sheet setting too.
- **Sidebar can now collapse fully.** Clicking the active rail tab (Loadouts / Workbench / Statistics) again collapses the left rail entirely so the timeline + sheet take the full width. Click any tab to bring it back. The sidebar's open/closed state is remembered alongside its mode.
- **Combat sheet header rebuilt — operator cards are clickable.** The operator cards in the new sheet header are now interactive: click one to open its loadout edit panel, and the card highlights to show which operator you're editing. The team-stats strip at the bottom adds a **CC %** (Crowd Control time as a fraction of fight duration) next to Team DPS / Duration / Time to Kill / Team Total.
- **Top bar slimmed down.** Removed the inline loadout-name display, the rename pencil, and the **Clear All** button — those operations live on the loadout itself in the sidebar (right-click → rename / delete). The bar now just hosts global app affordances (theme, settings, devlog, freeform, collab, keyboard shortcuts).
- **Create / Edit Views modal: dual-mode confirm button.** The button at the bottom now reads **Generate N views** when you have axes selected, **Clear views** (red, with a tooltip) when you've collapsed every selection back down to the parent loadout, or just **Generate** if no permutations would be created yet. Lets you delete an existing view set without leaving the modal.
- **Export modal: drag to sweep-invert.** In the Export Loadouts picker you can now click and drag across rows to invert their selection state as the cursor passes over them — useful for quickly toggling a contiguous block. Click still toggles a single row; Shift-click still selects a range.
- **Invalid permutation tooltips show what's wrong.** Hovering the warning ⚠ on a permutation view used to just say "open it for details". It now lists every placement issue inline (e.g. "Antal: weapon rank R1 not unlocked at potential 0").
- **Custom skill editor: hover-help on every field.** Hovering a field label in the skill / segment / frame / clause editors now reveals a short explanation of what the field controls — what `eventCategoryType` does, what `ANIMATION` vs `COOLDOWN` vs `STASIS` mean, when each clause variant fires, etc. Lowers the onboarding cost for building a custom skill from scratch.
- **Yvonne Cryoblasting Pistolier no longer leaks ultimate energy.** The ult was double-counting its own SP cost as natural recovery, slowly pumping ultimate energy during its own cast. The ultimate now correctly ignores its own energy bookkeeping.
- **Yvonne crit-stack and reaction-consume frames consume max stacks at once.** Cryoblasting Pistolier's per-tick stack draws — Crit Rate, Crit Damage, and Solidification consumption — now collapse to a single max-stack consume per frame instead of decrementing one at a time, matching the in-game behavior.
- **Rossi Seething Blood now triggers on critical hits.** The talent's predicate (was: "DEAL CRITICAL DAMAGE") is rewritten as the cleaner "PERFORM CRITICAL_HIT" — same trigger semantics, but now visible to the rest of the trigger pipeline. Talent damage frame correctly tagged Heat element.
- **Internal cleanup.** Removed unused segment subtypes (NORMAL / ACTIVE / INPUT_DELAY) from the engine vocabulary, normalised dozens of operator JSONs to drop redundant `determiner: THIS` defaults and to express CONSUME-many-stacks effects with explicit `with: {stacks: MAX}`. No visible behaviour change on its own — these unblock further engine work.
